    ...
    <div className="rounded-2xl border border-border bg-surface p-5 sm:p-6 space-y-5 sticky top-28">
      ...
      <form onSubmit={applyCoupon} className="space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="submit"
            className="h-11 px-4 rounded-xl border border-border text-xs font-semibold hover:bg-muted shrink-0"
          >
            اعمال
          </button>
        </div>
        {couponError && (
          <p className="text-[11px] text-[var(--discount)]">{couponError}</p>
        )}
        {applied && (
        ...
        <p className="text-[10px] text-muted-foreground">
          نمونه: WEBSPEED10 · SALE20 · WELCOME
        </p>
      </form>
      ...
      <div className="space-y-3 text-sm border-t border-border pt-4">
        <div className="flex justify-between">
          <span className="text-muted-foreground">
            جمع ({formatNumber(totalItems)} کالا)
          </span>
      ...
      <div className="rounded-xl bg-muted/70 p-3.5 space-y-2">
        ...
        <div className="h-1.5 rounded-full bg-border overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min(100, (payable / freeShipThreshold) * 100)}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="flex justify-between items-baseline pt-1 border-t border-border">
        ...
        <span className="text-lg font-bold num" data-num>
          {formatPrice(payable)}
        </span>
      </div>
      ...
      <Link
        href="/checkout"
        className="flex h-12 w-full items-center justify-center rounded-xl bg-foreground text-background text-sm font-semibold hover:bg-foreground/90 transition-colors"
      >
        ادامه فرآیند خرید
      </Link>
      <Link
